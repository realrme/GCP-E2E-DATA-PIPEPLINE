...
#     → dbt_run_bronze    (dbt run --select models/bronze)
#     → dbt_run_silver    (dbt run --select models/silver)
#     → dbt_snapshot_scd  (dbt snapshot — SCD Type 2)
#     → dbt_run_gold      (dbt run --select models/gold)
#     → end
# ============================================================
...
with DAG(
    ...
    def batch_ingest_postgres_to_bq():
        """
        Batch ingestion task — pulls rows from every table listed in
        tables_config.json out of the local Docker 'postgres_source' service,
        stamps metadata, and loads them into the BigQuery bronze dataset.
        """
